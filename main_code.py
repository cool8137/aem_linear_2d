...
def make_auto_BC_com_mat(ele_com_mat):
    ...
    # first row is defined so as to define the shape of matrix.
    # don't forget to delete the first row of spr_com_mat
    spr_com_mat = numpy.matrix([[0,0,0,0,0,0,0,0,0]])
    spr_no = 0
    ...
    for i in range(len(ele_com_mat)):
        ...
        for j in range(n):
            ...
            # Stack all info into the spr_com_mat matrix
            spr_com_mat = numpy.vstack([spr_com_mat, [spr_no,ele_1,edg_1,BC,d,x_1,y_1,x_2,y_2]])
            spr_no = spr_no + 1
...
BC_com_mat = make_auto_BC_com_mat(numpy.array([[element[0],4,3,20]]))
...
loads = numpy.hstack([numpy.zeros((1,(3*n-3))),[[0,-1000,0]]]).T
...
loads2 = numpy.array([[10000,0,0]]).T
...
ele_com_mat3 = numpy.array([[element3[0],element3[1],2,4,20]])

spr_com_mat3 = make_auto_spr_com_mat(ele_com_mat3)

spr_array3 = make_spring_array(spr_com_mat3)

BC_com_mat3 = make_auto_BC_com_mat(numpy.array([[element3[0],4,3,2]]))
...
loads3 = numpy.array([[0,0,0,1000,0,0]]).T

K3=glob_mat3

dis3 = numpy.linalg.solve(K3,loads3)
